...
void debug_log(const char* error_msg, cl_program program, cl_device_id device_id, cl_build_status status, cl_int ret){
    if(ret != CL_SUCCESS) {
        ...
        clGetProgramBuildInfo(program, device_id,
                              CL_PROGRAM_BUILD_LOG, logSize + 1, log, NULL);

        printf("Build failed; error=%d(%s), status=%d, programLog:\n%s\n", ret, getErrorString(ret), status, log);
        exit(1);
    }
}
...
void check_error(const char* error_msg, cl_int ret){
    if(ret != CL_SUCCESS) {
        printf("OpenCL failed; error=%d(%s) msg=%s\n", ret, getErrorString(ret), error_msg);
        exit(1);
    }
}